# DumpO: A tool to dump C-Code for Neural Network inference on heterogeneous platforms #

Dumpo is a tool designed to facilitate the deployment of ONNX-Graphs to C-programmable platforms.
...
The main mechanisms that enable this in DumpO is a hierarchical separation of responsibilities. 
This document is meant to outline the general flow used in DumpO and also highlight where users can
...
DumpO is generally frontend agnostic - currently, only ONNX graphs are supported and the parsers that are implemented 
...
operations as found in DumpO. The middleware is explicitly NOT responsible for scheduling, tiling, typechecking and strict parsing of the input graph.
...
The backend is responsible for taking a node-by-node deployable ONNX graph and lowering it to C-Code. To this end, DumpO
...
The main user object in DumpO are platforms. In essence, a platform provides DumpO the information for the following:
...
3. A set of Variable, Constant and StructBuffer types - These objects tell DumpO how to generate allocation and deallocation code
4. An optimizer - this object is DumpO's middleware and transforms a graph to something deployable
...
The parsing steps in DumpO is designed to ensure that a graph can be run on the target platform and to make sure that the
node's constant inputs (if there are any) are known at compile time. This is done using the NodeParser class in DumpO.
...
Type Checking is not only best practice, but necessary to bind the correct kernels. DumpO internally represents
every tensor, i.e. inputs and outputs of operations with a number of Levels, the logarithm of which is the lowest
number of bits required to represent the tensor. Strict type checking support in DumpO requires that every input and output
of every kernel binding is annotated with a data type. DumpO will then choose the first matching kernel binding 
that is available and fulfills the bitwidth constraints. DumpO will then propagate the new constraints given by the 
corresponding data type on the output nodes. In this way, DumpO ensures that not only every operation is executable but also
the full graph.
...
Binding is the process of of translating a layer's representation to a C-Kernel. To facilitate this, DumpO bundles 
