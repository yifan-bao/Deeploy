...
class PULPRQAddTemplate(NodeTemplate):
    ...
    def alignToContext(self, ctxt: NetworkContext, nodeRep: Dict) -> Tuple[NetworkContext, Dict, List[str]]:
        ...
        signedI2 = ctxt.lookup(nodeRep['data_in_2'])._type.referencedType.typeMin < 0
        signedI = ctxt.lookup(nodeRep['data_in_1'])._type.referencedType.typeMin < 0
        signedO = ctxt.lookup(nodeRep['data_out'])._type.referencedType.typeMin < 0
        nodeRep['input_2_signed'] = signedI2
