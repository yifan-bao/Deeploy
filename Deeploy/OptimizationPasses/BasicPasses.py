...
def merge_matmul_add_fun(ctxt: NetworkContext, graph: gs.Graph, match: Match, name: str):
    matched_nodes = [m for k, m in match.nodes_map.items()]
...
class MatMulAddMergePass(ReplaceSequentialPatternPass):
    ...
    def __init__(self):
        ...
        name = f"_MERGE_MATMUL_ADD_PASS"
        super().__init__(graph, merge_matmul_add_fun, name)
