    ...
    for idx, node in enumerate(graph.nodes):
        ...
        if node.op == "RequantizedGemm":
            ...
            if node.attrs['transA'] != 0 and isinstance(inputA, gs.ir.tensor.Variable):
                ...
                inputATransposeOutput = gs.Variable("MatMulTransposeIn" + str(transposeIdx),
                                                    dtype = np.float32,
                                                    shape = outShapeA)
                inputATransposeNode = gs.Node(name = 'MatMulTranspose' + str(transposeIdx),
                                              op = "Transpose",
                                              inputs = [inputA],
                                              outputs = [inputATransposeOutput],
                                              attrs = {'perm': permute})
            ...
            if node.attrs['transB'] != 1 and isinstance(inputB, gs.ir.tensor.Variable):
                ...
                inputBTransposeOutput = gs.Variable("MatMulTransposeIn" + str(transposeIdx),
                                                    dtype = np.float32,
                                                    shape = outShapeB)
                inputBTransposeNode = gs.Node(name = 'MatMulTranspose' + str(transposeIdx),
                                              op = "Transpose",
                                              inputs = [inputB],
                                              outputs = [inputBTransposeOutput],
                                              attrs = {'perm': permute})
...
def NCHWtoNHWC(graph: gs.Graph, default_channels_first: bool = True):
    ...
    for idx, node in enumerate(graph.nodes):
        ...
        if (node.op == "RequantizedConv" or node.op == "MaxPool" or node.op == "Pad") and (
            ("channels_first" in node.attrs and node.attrs["channels_first"] != default_channels_first) or
            ("channels_first" not in node.attrs and default_channels_first == 0)):
            ...
            inputTransposeOutput = gs.Variable("TransposeIn" + str(transposeIdx),
                                               dtype = np.float32,
                                               shape = newShape(inputNode, np.array(inPermute)))
            outputTransposeInput = gs.Variable("TransposeOut" + str(transposeIdx + 1),
                                               dtype = np.float32,
                                               shape = newShape(outputNode, np.array(inPermute)))

            inputTransposeNode = gs.Node(name = 'Transpose' + str(transposeIdx),
                                         op = "Transpose",
                                         inputs = [inputNode],
                                         outputs = [inputTransposeOutput],
                                         attrs = {'perm': inPermute})
            outputTransposeNode = gs.Node(name = 'Transpose' + str(transposeIdx + 1),
                                          op = "Transpose",
                                          inputs = [outputTransposeInput],
                                          outputs = [outputNode],
                                          attrs = {'perm': outPermute})
            ...
            if node.op == "RequantizedConv":
                ...
                if node.attrs['group'] == 1:
                    ...
                    weightTransposeOutput = gs.Variable("TransposeWeight" + str(transposeIdx + 2),
                                                        dtype = np.float32,
                                                        shape = newShape(weightNode, np.array(inPermute)))
                    weightTransposeNode = gs.Node(name = 'Transpose' + str(transposeIdx + 2),
                                                  op = "Transpose",
                                                  inputs = [weightNode],
                                                  outputs = [weightTransposeOutput],
                                                  attrs = {'perm': inPermute})
                    ...
                    weightTransposeOutput = gs.Variable("TransposeWeight" + str(transposeIdx + 2),
                                                        dtype = np.float32,
                                                        shape = newShape(weightNode, np.array(DWPermute)))
                    weightTransposeNode = gs.Node(name = 'Transpose' + str(transposeIdx + 2),
                                                  op = "Transpose",
                                                  inputs = [weightNode],
                                                  outputs = [weightTransposeOutput],
                                                  attrs = {'perm': DWPermute})
                    node.inputs[1] = weightTransposeOutput
                    graph.nodes.append(weightTransposeNode)
