...
from Deeploy.DeeployTypes import NetworkContext, NetworkOptimizationPass
from Deeploy.OptimizationPasses.BindingsOptimizationPasses.bindingUtils import bypassNode, editAttribute
from Deeploy.OptimizationPasses.BindingsOptimizationPasses.PassClasses import bindingaware
from Deeploy.OptimizationPasses.Matchers import BranchingMatcher, Match, NonBranchingMatcher, SubgraphMatcher
from Deeploy.OptimizationPasses.PassClasses import ReplaceSequentialPatternPass, SequentialPass
...
def _merge_transposeRequantInputs_fun(ctxt: NetworkContext, layerBinding, match: Match, name: str):
    ...
    mergeCondition = lambda node: all([
        ...
        len(node.outputs[0].outputs) == 1,
        rqNode.engineColor.engine.getTargetMemoryLevel(rqNode, node.outputs[0].name) == "L1",
    ...
    if not hasattr(rqNode, 'engineColor'):
        return ctxt, layerBinding
...
def _merge_transposeRequantOutputs_fun(ctxt: NetworkContext, layerBinding, match: Match, name: str):
    ...
    mergeCondition = lambda node: all([
        node.op == "Transpose", not "bypass" in node.attrs.keys(),
        len(rqNode.outputs[0].outputs) == 1,
        rqNode.engineColor.engine.getTargetMemoryLevel(rqNode, rqNode.outputs[0].name) == "L1",
