...
from Deeploy.DeeployTypes import ExecutionBlock, NetworkContext, NodeTemplate
...
class IntrospectiveCodeTransformationMixIn():
    ...
    def extractDynamicReferences(self,
        ...
        for template, nodeRep in executionBlock.nodeTemplates:
            newRefs = self._extractDynamicExpressions(ctxt, nodeRep, template, unrollStructs)
    ...
    def _fixCtxtOrdering(ctxt: NetworkContext, nameList: List[str]) -> List[str]:
        ...
        orderList = list(ctxt.globalObjects.keys()) + list(ctxt.localObjects.keys())
    ...
    def _extractDynamicExpressions(self,
        ...
        # Filter expressions for variables contained in nodeRep
        makoReferences = [
            node for node in makoExpressions
            if ((node in nodeRep) and type(nodeRep[node]) == str and (nodeRep[node] in ctxt.localObjects.keys()))
        ]

        def _unrollStructReferences(val) -> List[str]:
            # Unroll struct references
            structReferences = []
            if issubclass(type(val), StructClass):
                for key, _type in val.value.items():
                    if issubclass(type(_type), StructClass):
                        structReferences += _unrollStructReferences(val.value[key])
                    elif issubclass(type(_type), PointerClass) and val.value[key].referenceName != _NULL:
        ...
        for ref in makoReferences:
            references.append(nodeRep[ref])
            if unrollStructs:
                if (ctxt.is_local(nodeRep[ref]) or ctxt.is_global(nodeRep[ref])) and hasattr(
                        ctxt.lookup(nodeRep[ref]), "structDict"):
                    structReferences += _unrollStructReferences(ctxt.lookup(nodeRep[ref]).structDict)
