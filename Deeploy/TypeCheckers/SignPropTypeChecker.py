...
from Deeploy.DeeployTypes import ConstantBuffer, NetworkContext, NodeTypeChecker, VariableBuffer
...
class SignPropTypeChecker(NodeTypeChecker):
    ...
    def _inferSignedness(self, inputs: List[VariableBuffer], parserDict: Dict) -> Optional[List[int]]:
        return None
    ...
    def typeInferOutput(self, ctxt: NetworkContext, node: gs.Node, parserDict: Dict) -> NetworkContext:
        ctxt = super().typeInferOutput(ctxt, node, parserDict)
        ...
        if signProp:
            nLevels = self._inferNumLevels(inputs, parserDict)
            signedness = self._inferSignedness(inputs, parserDict)
