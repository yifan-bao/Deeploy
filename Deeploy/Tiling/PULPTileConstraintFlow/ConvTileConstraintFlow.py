...
class Conv2DTileConstraintFlow(TileConstraintFlow):
    ...
    def computeInputCube(kernelShape: Tuple[int, ...], pads: Tuple[int, ...], strides: Tuple[int, ...],
        ...
        leftMargin, rightMargin, topMargin, bottomMargin = Conv2DTileConstraintFlow.computeMargins(kernelShape)

        padding_left = (WOffset == 0) * pads[0]
        padding_top = (HOffset == 0) * pads[1]
        padding_bottom = (HOffset + HSize == outputDims[1]) * pads[2]
        padding_right = (WOffset + WSize == outputDims[2]) * pads[3]

        inputHOffset = HOffset * strides[0] - leftMargin * (HOffset != 0)
        inputWOffset = WOffset * strides[1] - topMargin * (WOffset != 0)
    ...
    def serializeTilingSolution(cls, tilingSolution: NodeMemoryConstraint, outputCubes: List[HyperRectangle],
        ...
        for cube in outputCubes:
            ...
            InCube, padding_tuple = Conv2DTileConstraintFlow.computeInputCube((weightH, weightW), pads, strides,
                                                                              weightC, cube,
