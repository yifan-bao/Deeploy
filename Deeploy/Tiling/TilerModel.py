...
from Deeploy.MemoryLevels import MemoryLevel
...
class TilerModel():
    ...
    def __init__(self, copyIdxSuffix: Optional[str] = None):
        ...
        self._memoryConstraints: List[Tuple[MemoryLevel, IntVar]] = []
    ...
    def addConstraint(self, constraintExpression: IntExpr, memoryLevel: Optional[MemoryLevel] = None):
    ...
    def addMinTileSizeConstraint(self,
        ...
        tileSizeVar = self.addVariable(prefix + nodeRep["nodeName"] + f"_{variableName}" + "_tileSize", 1,
                                       nodeRep[variableName])
        mulVar = self.addVariable(prefix + nodeRep["nodeName"] + f"_{variableName}" + "_mul", 1, nodeRep[variableName])
        addVar = self.addVariable(prefix + nodeRep["nodeName"] + f"_{variableName}" + "_add", 0, nodeRep[variableName])
        self.addConstraint(addVar <= tileSizeVar)
        self.addConstraint(addVar >= (modulo * (tileSizeVar < intvar.Max())))
        self.addConstraint(nodeRep[variableName] == mulVar * tileSizeVar + addVar)
        self.addConstraint(intvar == tileSizeVar)
    ...
    def addTileSizeDivisibleConstraint(self,
        ...
        mulMulVar = self.addVariable(prefix + nodeRep["nodeName"] + f"_{variableName}" + "_mulmul", 1,
                                     nodeRep[variableName])

        addVar = self.addVariable(prefix + nodeRep["nodeName"] + f"_{variableName}" + "_add", 0, nodeRep[variableName])
        addMulVar = self.addVariable(prefix + nodeRep["nodeName"] + f"_{variableName}" + "_muladd", 0,
                                     nodeRep[variableName])

        self.addConstraint(addVar <= tileSizeVar)
        self.addConstraint(addVar == modulo * addMulVar)
        self.addConstraint(tileSizeVar == modulo * mulMulVar)
        self.addConstraint(nodeRep[variableName] == mulVar * tileSizeVar + addVar)
        self.addConstraint(intvar == tileSizeVar)
    ...
    def debugConstraints(self) -> bool:
        ...
        if offendingGeometricalConstraints != []:
            ...
            errorMsg += [str(offendingGeometricalConstraints)]
            raise RuntimeError(("\n").join(errorMsg))
    ...
    def _trySetupConstraints(self,) -> bool:
        ...
        return self._model.CheckConstraint(constraint)
    ...
    def _solveModel(self, solType: Union[Literal['min'], Literal['max']] = 'max') -> SolutionCollector:
        ...
        _ = self._model.Solve(decision_builder, [objective, collector])
